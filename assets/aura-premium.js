...
const Aura = {
  ...
  toggleWishlist(handle) {
    let list = this.getStorage(this.KEYS.WISHLIST);
  ...
  toggleCompare(handle, specs) {
    let list = this.getStorage(this.KEYS.COMPARE);
  ...
  async openQuickAdd(productUrl) {
    ...
    try {
      ...
      content.innerHTML = '<div class="aura-error">Error loading Boutique Experience. Please try again.</div>';
    }
  },
  ...
  init() {
    this.updateUI();
    this.bindEvents();
    console.log('Aura Core Initialized');
  },

  updateUI() {
    const wishlist = this.getStorage(this.KEYS.WISHLIST);
    const compare = this.getStorage(this.KEYS.COMPARE);
...
window.openQuickAdd = (url) => Aura.openQuickAdd(url);
window.formatMoney = (cents, format) => Aura.formatMoney(cents, format);
window.updateQvQty = (btn, delta) => Aura.updateQvQty(btn, delta);
window.addQvToCart = (btn) => Aura.addQvToCart(btn);
window.buyQvNow = (btn) => Aura.buyQvNow(btn);
